...
static constexpr int kLyricsPanelWidth = 520; 
static constexpr int kLyricsPanelPaddingLeft = 40;
...
protected:
    ...
    void paintGL() override {
        ...
        // Fully transparent clear color, so it naturally blends with anything below if opacity < 1.0
...
FullscreenPlayer::FullscreenPlayer(QWidget *parent) : QWidget(parent)
{
    ...
    auto makeTitleBtn = [&](const QString &svg) {
        ...
        b->setStyleSheet("QPushButton{background:transparent;border:none;color:rgba(255,255,255,0.7);font-size:18px;}QPushButton:hover{color:white;}");
        b->setCursor(Qt::PointingHandCursor);
        b->setText(svg);
        return b;
    };

    QPushButton *downBtn = makeTitleBtn(QString::fromUtf8("\xE2\x8C\xAC"));
    connect(downBtn, &QPushButton::clicked, this, &FullscreenPlayer::closeOverlay);
    tbl->addWidget(downBtn);
    tbl->addStretch();
    
    m_closeBtn = makeTitleBtn(QString::fromUtf8("\xE2\x9C\x95"));
    ...
    lyricsLayout->setContentsMargins(kLyricsPanelPaddingLeft, 60, 40, 60);
    ...
    // Separated Seekbar Area (Always visible)
    ...
    // Playback Controls (Fades in on hover)
    ...
    // Explicit individual opacity effects to avoid QGraphicsOpacityEffect on QOpenGLWidget rendering bug
    m_centerAreaOpacityEffect = nullptr;
    //m_centerArea->setGraphicsEffect(m_centerAreaOpacityEffect);
    
    m_titleBarOpacityEffect = nullptr;
    //m_titleBar->setGraphicsEffect(m_titleBarOpacityEffect);
    
    m_seekBarOpacityEffect = nullptr;
    //m_seekBarArea->setGraphicsEffect(m_seekBarOpacityEffect);
...
bool FullscreenPlayer::eventFilter(QObject *w, QEvent *e) {
    if (m_lyricsList && w == m_lyricsList->viewport() && (e->type() == QEvent::Wheel || e->type() == QEvent::MouseButtonPress || e->type() == QEvent::MouseMove || e->type() == QEvent::KeyPress)) { suspendLyricsAutoScroll(); }
    return QWidget::eventFilter(w, e);
}

void FullscreenPlayer::setMainUiOpacity(qreal v) {
    m_mainUiOpacity = v;
    if (m_centerAreaOpacityEffect) m_centerAreaOpacityEffect->setOpacity(v);
    if (m_titleBarOpacityEffect) m_titleBarOpacityEffect->setOpacity(v);
    if (m_seekBarOpacityEffect) m_seekBarOpacityEffect->setOpacity(v);
}
...
void FullscreenPlayer::openFor(const QPixmap &cover, const QString &title, const QString &artist, const QString &album, int durationMs, int positionMs, bool isPlaying, int volume) {
    ...
    setMainUiOpacity(0.0);
    if (m_bgWidget) m_bgWidget->setOpacity(0.0f);
    
    m_openCloseAnim = new QParallelAnimationGroup(this);
    
    auto *oa = new QPropertyAnimation(this, "mainUiOpacity", m_openCloseAnim);
    oa->setStartValue(0.0); oa->setEndValue(1.0); oa->setDuration(350); oa->setEasingCurve(QEasingCurve::OutCubic);
    ...
    if (m_bgWidget) {
        auto *bgAnim = new QVariantAnimation(m_openCloseAnim);
        bgAnim->setStartValue(0.0f); bgAnim->setEndValue(1.0f); bgAnim->setDuration(350); bgAnim->setEasingCurve(QEasingCurve::OutCubic);
        connect(bgAnim, &QVariantAnimation::valueChanged, this, [this](const QVariant &v) {
            if (m_bgWidget) m_bgWidget->setOpacity(v.toFloat());
        });
        m_openCloseAnim->addAnimation(bgAnim);
    }
...
void FullscreenPlayer::closeOverlay() {
    ...
    if (m_bgWidget) {
        auto *bgAnim = new QVariantAnimation(m_openCloseAnim);
        bgAnim->setStartValue(m_bgWidget->opacity()); bgAnim->setEndValue(0.0f); bgAnim->setDuration(250); bgAnim->setEasingCurve(QEasingCurve::InQuad);
        connect(bgAnim, &QVariantAnimation::valueChanged, this, [this](const QVariant &v) {
            if (m_bgWidget) m_bgWidget->setOpacity(v.toFloat());
        });
        m_openCloseAnim->addAnimation(bgAnim);
...
void FullscreenPlayer::updateTrack(const QPixmap &cover, const QString &title, const QString &artist, const QString &album, int durationMs) {
    ...
    if (isVisible()) { extractPalette(m_rawCover); if (m_bgWidget) { m_bgWidget->setPalette(m_palette); if (m_paletteTransitionAnim) { m_paletteTransitionAnim->stop(); m_paletteTransitionAnim->start(); } } }
...
void FullscreenPlayer::applyLyrics(const QString &synced, const QString &plain, bool instrumental) {
    (void)instrumental; // Fix unused parameter warning
...
void FullscreenPlayer::paintEvent(QPaintEvent *) { QPainter painter(this); painter.setRenderHint(QPainter::SmoothPixmapTransform); painter.fillRect(rect(), QColor(18, 18, 18, 0)); }
void FullscreenPlayer::keyPressEvent(QKeyEvent *e) { if (e->key() == Qt::Key_Escape) closeOverlay(); else QWidget::keyPressEvent(e); }
void FullscreenPlayer::resizeEvent(QResizeEvent *e) { QWidget::resizeEvent(e); if (m_bgWidget) m_bgWidget->setGeometry(rect()); if (m_isOpen) updateLayout(); }
...
void FullscreenPlayer::updateLayout() {
    ...
    centerPos += m_centerOffset.toPoint();
    ...
    // Pin seekbar area directly to the bottom
    ...
    if (m_playbackControlsSlideAnim && m_playbackControlsSlideAnim->state() == QAbstractAnimation::Stopped) {
        // Playback controls sit just above the seekbar area
        int baseY = h - sbH - pcH;
...
void FullscreenPlayer::updateState() {
    double tx = m_lyricsVisible ? -220 : (m_stateHinted ? -50 : 0);
...
void FullscreenPlayer::animateTick() {
    ...
    if (isVisible()) {
        ...
        int w = width(), h = height();
        // Generous bounds check to catch mice thrown against the absolute edge of the screen
        bool isMouseInside = (pos.x() >= -2 && pos.x() <= w + 2 && pos.y() >= -2 && pos.y() <= h + 2);
        
        if (isMouseInside && !m_userSeeking) {
            bool inBottom = (h - pos.y()) <= h * 0.40;
