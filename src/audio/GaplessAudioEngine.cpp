...
HSTREAM GaplessAudioEngine::createStream(const QString &filePath)
{
    ...
    HSTREAM fxStream = BASS_FX_TempoCreate(decodeStream, BASS_FX_FREESOURCE);
    if (!fxStream) {
...
{
#ifdef MUSICPLAYER_HAS_BASS
    if (m_activeStream && BASS_ChannelIsActive(m_activeStream) == BASS_ACTIVE_PLAYING) {
        BASS_ChannelPause(m_activeStream);
        updateState(Paused);
    }
#endif
}
...
{
#ifdef MUSICPLAYER_HAS_BASS
    if (m_activeStream && BASS_ChannelIsActive(m_activeStream) == BASS_ACTIVE_PAUSED) {
        BASS_ChannelPlay(m_activeStream, FALSE);
        updateState(Playing);
    }
#endif
...
#ifdef MUSICPLAYER_HAS_BASS
    if (m_activeStream) {
        BASS_ChannelStop(m_activeStream);
...
void GaplessAudioEngine::seek(qint64 positionMs)
{
    ...
    if (m_activeStream) {
        ...
        BASS_ChannelSetPosition(m_activeStream, posBytes, BASS_POS_BYTE);
...
#ifdef MUSICPLAYER_HAS_BASS
    if (m_activeStream) {
        qint64 posBytes = BASS_ChannelGetPosition(m_activeStream, BASS_POS_BYTE);
...
{
#ifdef MUSICPLAYER_HAS_BASS
    if (m_state == Playing && m_activeStream) {
        emit positionChanged(position());
        
        // Calculate audio level for visualizer
        DWORD level = BASS_ChannelGetLevel(m_activeStream);
...
void GaplessAudioEngine::processPendingTransitions()
{
    ...
    if (m_nextStream) {
        ...
        if (BASS_ChannelIsActive(m_activeStream) != BASS_ACTIVE_PLAYING || m_crossfadeDurationMs > 0) {
            if (m_crossfadeDurationMs == 0) {
                BASS_ChannelStop(m_activeStream);
            }
            // For crossfade, BASS will automatically stop `m_activeStream` when volume reaches -1 if we set it so, 
            // but -1 means fade to 0 and stop according to BASS docs.
            
            // HSTREAM oldStream = m_activeStream;
            m_activeStream = m_nextStream;
            m_nextStream = 0;
            m_currentFilePath = m_preparedNextPath;
            m_preparedNextPath.clear();
            
            // Apply normal volume just in case crossfade didn't finish or wasn't used
        ...
        if (BASS_ChannelIsActive(m_activeStream) == BASS_ACTIVE_STOPPED) {
            updateState(Stopped);
